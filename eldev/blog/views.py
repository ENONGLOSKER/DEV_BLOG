...
def master(request):
    ...
    if cari:
        page = blogPost.objects.filter(Q(judul__icontains=cari) | Q(kategori__icontains=cari))
    else:
        page = blogPost.objects.all().order_by('-id')

    hal = Paginator(page, 5)
    hal_list = request.GET.get('page')
    page = hal.get_page(hal_list)

    jlh_pos = blogPost.objects.all().count()
    # jlh_kategori = blogPost.values('kategori').distinct().count()

    context = {
        'judul':'PAGE MASTER',  
        'datas':page,     
        'jlh_pos':jlh_pos,
        # 'jlh_kategori':jlh_kategori,
    }
